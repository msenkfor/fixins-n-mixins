...
const recipeToolDefinition: Anthropic.Messages.Tool = {
  name: "save_recipes",
  description:
            ...
            ingredients: {
              ...
              items: {
                ...
                properties: {
                  ...
                  quantity: {
                    type: "string",
                    description: "Numeric quantity as string (e.g. '2', '0.5')",
                  },
                  unit: {
                    type: "string",
                    description:
...
Use the save_recipes tool to return your recipes.`,
    ...
    if (!toolUseBlock) {
      ...
      return new Response(
        JSON.stringify({ error: "AI model did not return structured recipes" }),
        {
          status: 502,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
    ...
    const input = toolUseBlock.input as { recipes: Omit<Recipe, "id" | "matchedIngredientCount" | "totalIngredientCount">[] };
    ...
    const recipes: Recipe[] = input.recipes.map((raw, index) => {
      const matchedCount = raw.ingredients.filter((ing) => {
        // Check if this ingredient name matches any from the user's photo
    ...
    const novelRecipes = recipes.filter(
      (r) =>
        !shown.some(
          (title) => title.toLowerCase().trim() === r.title.toLowerCase().trim(),
        ),
    );
